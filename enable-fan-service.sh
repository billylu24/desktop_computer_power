#!/usr/bin/env bash
# Final activation step. Run only after mapping/calibration and active PWM tests.
set -euo pipefail
...
grep -Eq '^[[:space:]]*enabled[[:space:]]*=[[:space:]]*true[[:space:]]*$' "$FANS_CONF" ||
  die 'fans.conf is not enabled; mapping and calibration must be completed first'
